...
function newHired(nomeCompleto) {
    const splitName = nomeCompleto.split(' ');
    let email = splitName[0];
    for(let index = 1; index < splitName.length; index +=1){
        email += `_${splitName[index]}`;
    }
    email += `@trybe.com`;
    const obj = {Nome: `${nomeCompleto}`, Email:`${email}`};
    return obj;
}
